  ...
  return (
    <div className="grid gap-8 pb-28 lg:grid-cols-[minmax(0,1fr)_22rem] 2xl:grid-cols-[minmax(0,64rem)_24rem] 2xl:justify-between lg:items-start lg:pb-0">
      <div className="space-y-8">
        <fieldset>
          <legend className="text-title">1. Pick a box size</legend>
          <div className="mt-4 grid gap-2 sm:grid-cols-3">
            {box.variants.map((v) => (
              <label
                key={v.id}
                className={`cursor-pointer border p-4 transition-colors has-[:focus-visible]:outline-2 has-[:focus-visible]:outline-offset-2 has-[:focus-visible]:outline-(--focus-ring) ${
                  v.id === sizeId ? "border-ink bg-ink text-on-ink" : "border-border bg-surface text-text hover:border-ink"
                }`}
              >
                <input type="radio" name="box-size" value={v.id} checked={v.id === sizeId} onChange={() => changeSize(v.id)} className="sr-only" />
                <span className="block text-xl font-light">{v.label}</span>
        ...
        <fieldset>
          <legend className="text-title">2. Choose your flavours</legend>
          <ul className="mt-4 grid gap-2 sm:grid-cols-2 2xl:grid-cols-3">
            {cookies.map((c) => {
              const count = counts[c.slug] ?? 0;
              return (
                <li key={c.slug} className="flex items-center gap-4 border border-border bg-surface p-2">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {c.eggless && <Badge className="mt-1">Eggless</Badge>}
                    <div className="mt-2">
      ...
      <aside id="your-box-panel" aria-labelledby="your-box" className="scroll-mt-24 border border-border bg-surface p-6 lg:sticky lg:top-28">
        <h2 id="your-box" className="text-title">
          3. Your box
        </h2>
        <p className="mt-1" aria-live="polite">
          {picks.length} of {capacity} picked
        </p>
        <ul className={`mt-4 grid gap-2 border-2 border-ink bg-beige-100 p-2 ${capacity === 12 ? "grid-cols-4" : capacity === 4 ? "grid-cols-2" : "grid-cols-3"}`}>
          {Array.from({ length: capacity }, (_, i) => {
            const slug = picks[i];
            const cookie = slug ? cookies.find((c) => c.slug === slug) : undefined;
            return (
              <li key={i} className={`aspect-square p-1 ${cookie ? "bg-beige-50" : "border border-dashed border-beige-500"}`}>
                {cookie ? (
        ...
        <div className="mt-4 flex items-baseline justify-between border-t border-border pt-4">
          ...
          <span className="text-2xl font-light text-text">{formatPrice(size.price)}</span>
        </div>
        ...
        <div id="box-status" role="status" className="mt-2 min-h-6">
          {error && <p className="font-semibold text-rose-deep">{error}</p>}
          {added && (
            <p>
              {added}{" "}
      ...
      <div className="bottom-bar fixed inset-x-0 bottom-0 z-20 border-t border-border bg-bg px-4 pt-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] lg:hidden">
        {error && <p className="mb-2 text-sm font-semibold text-rose-deep">{error}</p>}
        <div className="flex items-center gap-4">
          <a href="#your-box-panel" className={`min-w-0 flex-1 rounded-sm ${focusClass}`}>
            ...
            <span className="block text-xl font-light tabular-nums">{formatPrice(size.price)}</span>
          </a>
          {remaining > 0 ? (
